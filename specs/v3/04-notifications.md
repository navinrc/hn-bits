...
Telegram only in V3. Interface first, so Discord (or desktop notifications) later = one new file, zero watcher changes.
...
Watcher takes `Notifier[]` (V3: array of one). Discord later = new `discord.ts` implementing `Notifier`, enabled by config presence.
...
Setup (documented in README): create bot via `@BotFather` → token; message the bot once, get chat id via `getUpdates`. Both values required; missing/partial section = exit 2 from watcher ([03-watcher.md](03-watcher.md)).
