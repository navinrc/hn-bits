...
| 7: Cron auto-install | done | `src/lib/schedule.ts` (`hasScheduledJob`/`installScheduledJob`/`removeScheduledJob`, wraps `crontab -l`/`crontab -`, marker comment `# hn-bits watch`); `hn sub add` prompts to install on the first subscription if no job exists yet; `hn schedule status/install/remove` for manual management. `hn watch --once` itself unchanged. Matches amended `specs/v3/03-watcher.md` scheduling section. Verified live: install/status/idempotent-reinstall/remove against real crontab (frappe-erp entries untouched, restored identical to backup after test), and the first-`sub add`-prompts/second-add-doesn't fork tested via an isolated `$HN_BITS_DB` |
| 7b: Cron auto-install, TUI parity | done | Original CLI-only prompt missed the subs-tab add path (`SubscriptionsView`'s `a` key / search's `S` → `SubscriptionForm.tsx`, which calls `addSubscription` directly, bypassing `src/index.tsx`). `SubscriptionForm.tsx` now runs the same first-subscription/no-schedule check after a successful add, and — since it can't block on stdin like the CLI's `readline` prompt inside Ink's raw-mode input loop — shows an inline `y/n` confirm line (same pattern as `SubscriptionsView`'s existing delete confirm) before calling `onSave`. 3 new tests in `SubscriptionForm.test.tsx` (prompts when uninstalled, installs on `y`, skips on `n`/skips prompt entirely when already installed), `../lib/schedule.js` mocked module-wide same as `watch.test.ts`'s pattern |
| 7c: Cron toggle + status indicator, subs tab | done | The add-form prompt was still one-time only, with no way to check/toggle afterward except the CLI. `SubscriptionsView.tsx` gains a `c` key (`SUBS_KEYS` in `keymap.ts`, additive) calling `installScheduledJob`/`removeScheduledJob` directly (reversible toggle, no confirm — unlike delete), plus a persistent `schedule: installed (every 30 min)` / `schedule: not installed` status line (both the populated list and empty-subscriptions states) and a transient `useFlash` confirmation/failure message on top. Viewport math (`statusLineHeight`/`listHeight`) updated since the status line is always-on, not conditional like the delete confirm. 6 new tests in `SubscriptionsView.test.tsx`, `../lib/schedule.js` mocked the same way as `SubscriptionForm.test.tsx` (now required repo-wide for this file since every test mounts the component and would otherwise hit a real `crontab -l`) |
...
| Live polish (user-verified) | done | Two more live findings: (a) macOS silently drops notifications when the impersonated sender (Terminal.app) lacks notification permission: alerter still prints `@TIMEOUT`/exit 0, undetectable from the watcher; one-time System Settings grant documented in README + spec. (b) per-sub `--group` made same-run matches replace each other (collapsed stack opened only the newest URL): group is now `hn-<subId>-<storyId>`, each match keeps its own clickable notification. Both verified by re-triggering real matches (backup + seen-row delete + `last_run_at` reset for one sub, reviewer-agent-validated, rolled forward cleanly): two separate banners, each opening its own story URL |
